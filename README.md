...
```mermaid
graph LR
    A["📡 公开数据源<br/>福彩官方 · 500彩票网 · idcd.com"]
...
- **自动更新**：开奖日 Actions 多源抓取 → commit → Pages 部署
- **纯原生**：零框架、零构建、零依赖，push 即生效
...
│   ├── index.html
│   ├── css/                 # 模块化样式（8 文件）
│   ├── js/                  # ES Modules（8 文件 + utils/）
│   └── data/                # 静态 JSON（按年存储 + stats + latest + index）
├── scripts/                 # Python 数据脚本
│   ├── fetch_latest.py      # 多源抓取（福彩 / 500彩票 / idcd）
...
数据源（多源 fallback）：[福彩官方](https://www.cwl.gov.cn/ygkj/wqkjgg/ssq/) → [500 彩票网](https://datachart.500.com/ssq/) → [idcd.com](https://www.idcd.com/)。
